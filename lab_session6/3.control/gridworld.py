...
class RiverGridWorld(Gridworld):
    def getPossibleActions(self, state):
        if state == self.grid.terminalState:
            return ()
        x, y = state
        if type(self.grid[x][y]) == int:
            return ('exit',)
        return ('north', 'west', 'south', 'east', 'do nothing')

    def getTransitionStatesAndProbs(self, state, action):
        states_and_probs = super().getTransitionStatesAndProbs(state, action)

        if action == 'do nothing':
            states_and_probs.append((state, 1))

        new_states_and_probs = copy.copy(states_and_probs)
        for i in reversed(range(len(states_and_probs))):
            state, prob = states_and_probs[i]
            if state == (0, 1):
                new_states_and_probs.pop(i)
                new_states_and_probs.append(((1,1), prob * 0.2))
                new_states_and_probs.append(((2,1), prob * 0.5))
                new_states_and_probs.append(((3,1), prob * 0.3))
            if state == (1, 1):
                new_states_and_probs.pop(i)
                new_states_and_probs.append(((2,1), prob * 0.2))
                new_states_and_probs.append(((3,1), prob * 0.5))
                new_states_and_probs.append(((4,1), prob * 0.3))
            if state == (2, 1):
                new_states_and_probs.pop(i)
                new_states_and_probs.append(((3,1), prob * 0.2))
                new_states_and_probs.append(((4,1), prob * 0.8))
            if state == (3, 1):
                new_states_and_probs.pop(i)
                new_states_and_probs.append(((4,1), prob))
